...
namespace Open.Threading
{
    ...
    public class ReadWriteHelper<TKey> : DeferredCleanupBase
    {
        ...
        private bool Execute(TKey key, LockType type, Action<ReaderWriterLockSlim> closure, int? millisecondsTimeout = null, bool throwsOnTimeout = false)
        {
            ...
            object context;
            if (!ContextPool.TryTake(out context) || context == null)
                context = new Object();
        ...
        {
            T result;
            Read(key, out result, valueFactory, millisecondsTimeout, true);
            return result;
        }
        ...
        {
            T result;
            Write(key, out result, valueFactory, millisecondsTimeout, true);
            return result;
        }
        ...
        public bool ReadWriteConditionalOptimized<T>(
            TKey key, ref T result, Func<bool, bool> condition, Func<T> closure,
            int? millisecondsTimeout = null, bool throwsOnTimeout = false)
        {
            if (condition == null)
                throw new ArgumentNullException("condition");
        ...
        /// Then if necessary executes the condition with an upgradeable read lock before acquiring a write lock.
        /// </summary>
        /// <param name="condition">Returns true if it should execute the query Action.</param>
        /// <returns>Returns false if a timeout is reached.</returns>
        public bool ReadWriteConditionalOptimized(
            ...
            int? millisecondsTimeout = null, bool throwsOnTimeout = false)
        {
            ...
            return ReadWriteConditionalOptimized(key, isWriteLock => condition(), closure, millisecondsTimeout, throwsOnTimeout);
        }
        ...
        public bool ReadWriteConditionalOptimized<T>(
            ...
            int? millisecondsTimeout = null, bool throwsOnTimeout = false)
        {
            return ReadWriteConditionalOptimized(key, ref result, isWriteLock => condition(), closure, millisecondsTimeout, throwsOnTimeout);
        }
        ...
        private void CleanupInternal()
        {
            ...
            foreach (var key in Locks.Keys.ToArray())
            {
                ...
                ReaderWriterLockTracker tempLock;
                if (Locks.TryGetValue(key, out tempLock) && tempLock != null) // This should be accurate and synchronized since we keep these locks private.
                {
        ...
        private void ConcurrentBagTrim<T>(ConcurrentBag<T> target, int maxCount = 0, bool dispose = false, bool allowExceptions = true)
        {
            ...
            try
            {
                while (!target.IsEmpty && target.Count > maxCount && target.TryTake(out context))
                {
                    if (dispose)
                    {
                        var i = context as IDisposable;
                        if (i != null)
                        {
                            //d.Add(i);
                            try
